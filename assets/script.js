...
// FULL MOON - transparent sprite placed behind the main scene
...
function createMoonGlowTexture() {
  ...
  gradient.addColorStop(0, "rgba(255, 221, 145, 0.7)");
  gradient.addColorStop(0.42, "rgba(255, 196, 92, 0.22)");
  gradient.addColorStop(1, "rgba(255, 174, 60, 0)");
...
const moonTexture = new THREE.TextureLoader().load(
  "./assets/trang-tron-transparent.png",
);
if (THREE.sRGBEncoding) moonTexture.encoding = THREE.sRGBEncoding;
...
let moonBaseY = 18;
function updateMoonLayout() {
  const compact = window.innerWidth < 768;
  moonBaseY = compact ? 18.5 : 18;
  moonGroup.position.set(compact ? -7.5 : -12.5, moonBaseY, -18);
  moonSprite.scale.setScalar(compact ? 7.4 : 9.8);
  moonGlow.scale.setScalar(compact ? 12.5 : 16.5);
}
updateMoonLayout();

const moonLight = new THREE.PointLight(0xffd58a, 0.65, 50);
moonLight.position.set(-9, 18, -10);
scene.add(moonLight);
...
function onPointerUp(event) {
  ...
  if (intersects.length > 0) {
    ...
    wishImage.src = "./assets/trang-tron-transparent.png";
